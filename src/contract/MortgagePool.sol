...
contract MortgagePool is ReentrancyGuard {
    ...
    function getFee(uint256 parassetAssets,
    	...
    	uint256 bottom = oneYear.mul(1 * 10 ** 18);
    	return topOne.div(bottom).add(topTwo.div(bottom.mul(1 * 10 ** 18).mul(2)));
    }
    ...
    function getMortgageRate(uint256 mortgageAssets,
    	...
    	return parassetAssets.mul(tokenPrice).mul(1 * 10 ** 18).div(pTokenPrice.mul(mortgageAssets));
    }
    ...
    function getInfoRealTime(address mortgageToken,
        ...
        uint256 maxRateEther = maxRateNum.mul(0.01 * 10 ** 18);
        if (mortgageRate >= maxRateEther) {
            maxSubM = 0;
            maxAddP = 0;
        } else {
            maxSubM = pLedger.mortgageAssets.sub(pLedger.parassetAssets.mul(tokenPriceAmount).mul(1 * 10 ** 18).div(maxRateEther.mul(pTokenPrice)));
            maxAddP = pLedger.mortgageAssets.mul(pTokenPrice).mul(maxRateEther).div(uint256(1 * 10 ** 18).mul(tokenPriceAmount)).sub(pLedger.parassetAssets);
        }
    }
    ...
    function getDecimalConversion(address inputToken,
    	...
    	if (inputToken != address(uint160(0))) {
    		inputTokenDec = IERC20(inputToken).decimals();
    	}

    	if (outputToken != address(uint160(0))) {
    ...
    /// @param num market base interest rate(num = ? * 1 * 10 ** 18)
    ...
    function setLiquidationLine(address mortgageToken,
        ...
        liquidationLine[mortgageToken] = num.mul(0.01 * 10 ** 18);
    }
    ...
    function setMaxRate(address mortgageToken,
                        uint256 num) public onlyGovernance {
    	maxRate[mortgageToken] = num.mul(0.01 * 10 ** 18);
    }
    ...
    function setInfo(address uToken,
        ...
        require(underlyingToPToken[uToken] == address(uint160(0)), "Log:MortgagePool:underlyingToPToken");
        require(address(insurancePool) != address(uint160(0)), "Log:MortgagePool:0x0");
    ...
    function setGovernance() public {
        governance = pTokenFactory.getGovernance();
        require(governance != address(uint160(0)), "Log:MortgagePool:0x0");
    }
    ...
    function coin(address mortgageToken,
        ...
        uint256 tokenPrice;
        uint256 pTokenPrice;
        if (mortgageToken != address(uint160(0))) {
