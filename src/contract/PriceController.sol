...
contract PriceController {
    ...
    function getPriceForPToken(address token,
        ...
        if (token == address(uint160(0))) {
            ...
            return (1 * 10 ** 18, getDecimalConversion(uToken, avg, address(uint160(0))));
        } else if (uToken == address(uint160(0))) {
            ...
            return (getDecimalConversion(uToken, avg, address(uint160(0))), 1 * 10 ** 18);
        } else {
            ...
            if (checkNToken(token, uToken)) {
                ...
                return (avg1, getDecimalConversion(uToken, avg2, address(uint160(0))));
                ...
                return (avg2, getDecimalConversion(uToken, avg1, address(uint160(0))));
            } else {
